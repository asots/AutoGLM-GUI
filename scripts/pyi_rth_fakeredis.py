    ...
    def _patched_open(file, mode='r', *args, **kwargs):
        ...
        if isinstance(file, str) and 'fakeredis' in file and 'commands.json' in file:
            ...
            if not os.path.exists(file):
                ...
                commands_json_path = os.path.join(sys._MEIPASS, 'fakeredis', 'commands.json')
                if os.path.exists(commands_json_path):
                    print(f"[fakeredis-hook] Redirecting {file} -> {commands_json_path}", file=sys.stderr)
                    file = commands_json_path
                else:
                    print(f"[fakeredis-hook] WARNING: commands.json not found at {commands_json_path}", file=sys.stderr)
